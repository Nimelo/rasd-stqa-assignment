...
\newcounter{TestDescriptionCounter}
\renewcommand{\theTestDescriptionCounter}{\number\numexpr\value{TestDescriptionCounter}+1\relax}

\newcommand{\reqidd}[1]
{
	\textbf{#1--\theTestDescriptionCounter:}
	\stepcounter{TestDescriptionCounter}
}
...
\newenvironment{testcase}[4]
{
	...
	\begin{center}
		\begin{minipage}[t]{0.65\linewidth}
			\flushleft 
			\textbf{TEST--CASE--\number\numexpr\value{TestCaseCounter}:} #1
		\end{minipage}
		\begin{minipage}[t]{0.25\linewidth}
			\flushright 
			\textsc{Priority:} #2
