...
Glossary for this feature is in Table \ref{tab:user_requirements_glossary}.
...
%component ratings, such as benefit, penalty, cost, and risk (each rated on a 
%relative scale from a low of 1 to a high of 9).
Features described in this chapter relate to basic properties of a job controller. Only general requirements are mentioned here, more detailed properties and features are presented in next chapter, which discuss scheduling algorithm. Glossary for this feature is in Table \ref{tab:scheduler_requirements_glossary}.
...
%component ratings, such as benefit, penalty, cost, and risk (each rated on a 
%relative scale from a low of 1 to a high of 9).
Requirements in this chapter are specification of a scheduler algorithm mentioned in requirement \textbf{JBC-3}. Glossary for this feature is in Table \ref{tab:scheduler_requirements_glossary}.
...
Requirements from chapters \ref{chp:sc} and \ref{chp:sim} are base of diagrams and models from appendix. From the scenario of a project there were extracted three different actors for computation platform visible on Figure \ref{app:cp:actors}. For each of those generalized actors (User and IT supporter) distinguished use cases are shown respectively on Figure \ref{app:cp:ucs} and \ref{app:cp:itucs}. There is a relatively big amount of use cases for actors, although the requirements do not specify anything more about them. One of the most important part of the system is a job control system which were described by requirements in section \ref{sec:jcf}, from which a simplified algorithm were designed which also takes care about accounting of a job -- Figure \ref{app:cp:jobcontroller}.

There is only one actor of a simulation tool shown in Figure \ref{app:st:actors}. Idea of a simulation tool is to do a simulation of computation platform, which should provide information about accounting practices. In the simplest way this kind of behavior is shown in Figure \ref{app:st:idea}. Action starts with a configuration of a simulation, next the chain of event inside simulation tool is performed giving the output of a simulation described in chapter \ref{chp:sim}. The use cases of a simulation tool user are divided into two boundary: simulation tool and configuration file -- \ref{app:st:ucs}. User can either run a simulation with given parameters or change parameters of a configuration file, which is a quite big. The most important part of a simulation tool is understanding of a scheduler algorithm which is presented in Figure \ref{app:st:scheduer} and detailed described in chapter \ref{chp:sim}.  
...
\subsection{Computation Platform Diagrams}
	...
	\begin{figure}[!hbtp]
		\centering
		\includegraphics[width=\textwidth, angle=90, scale=1.2]{../ea_files/generatedImages/Computational_platform/ITSupporterUCs.png}
